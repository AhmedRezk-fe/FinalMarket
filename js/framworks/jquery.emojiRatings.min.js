...
! function(a, b, c, d) {
    ...
    a.extend(e.prototype, {
        ...
        setupStyles: function() {
            var a = "cursor:pointer;text-decoration:none;",
                b = "font-size:" + this.settings.fontSize + "px;",
                c = ".jqEmoji{" + a + b + "}";
            h.append("<style>" + c + "</style>")
        },
        renderEmojis: function() {
            for (var a, b = g(this.settings.emoji), c = this.settings.count, d = "<div class='jqEmoji-container'>", e = 0; c > e; e++) a = "<span class='jqEmoji' data-index='" + e + "'>" + "<i class='fa fa-star  fa-fw'></i>" + "</span>", d += a;
            d += "</div>", h.append(d)
        },
        clearEmojis: function() {
            h.find(".jqEmoji").each(function() {
                a(this).children("svg").removeClass("active")
            })
        },
        colorEmojis: function(a) {
            this.clearEmojis();
            for (var b = 0; a > b; b++) h.find(".jqEmoji").eq(b).children("svg").addClass("active")
        },
